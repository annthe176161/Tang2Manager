...
import type { Employee, DailyTimesheet, MonthlyPayrollRecord, InvoiceCategory } from '../../types';
...
import { salaryApi, invoiceApi } from '../../services/api';
import { 
  Camera, 
  Copy, 
  Check, 
...
export const SalaryManager: React.FC<SalaryManagerProps> = ({
  ...
  const [zipInvoiceCategories, setZipInvoiceCategories] = useState<InvoiceCategory[] | null>(null);
  ...
  // Tải tất cả ảnh gồm 2 thư mục [1_Luong_Nhan_Vien] và [2_Hoa_Don_Thang] vào 1 file ZIP gửi sếp
  const handleExportAllSalaryImagesZip = async () => {
    if (isExportingZip) return;
    setIsExportingZip(true);
    setZipProgress({ current: 0, total: employees.length + 2, name: 'Bắt đầu khởi tạo dữ liệu...' });

    try {
      const zip = new JSZip();
      const salaryFolder = zip.folder('1_Luong_Nhan_Vien');
      const invoiceFolder = zip.folder('2_Hoa_Don_Thang');

      // 1. Chụp Bảng Tổng Hợp Tiền Lương -> Folder 1
      setZipProgress({ current: 1, total: employees.length + 2, name: '📸 Bảng Tổng Hợp Tiền Lương' });
      await new Promise((r) => setTimeout(r, 90));

      const summaryEl = offscreenSummaryRef.current || summaryTableRef.current;
      if (summaryEl && salaryFolder) {
        const summaryBlob = await captureElementToBlob(summaryEl, 2.5);
        if (summaryBlob) {
          salaryFolder.file(`00_Bang_Tong_Hop_Luong_Thang_${selectedMonth}_${selectedYear}.png`, summaryBlob);
        }
      }

      // 2. Chụp chi tiết phiếu chấm công của từng nhân viên -> Folder 1
      for (let i = 0; i < employees.length; i++) {
        const emp = employees[i];
        setZipProgress({
          current: i + 2,
          total: employees.length + 2,
          name: `📸 Phiếu lương [${emp.fullName}] (${i + 1}/${employees.length})`,
        });
        ...
        if (offscreenSlipRef.current && salaryFolder) {
      ...
      // 3. Chụp Bảng Tổng Hợp Chi Phí Hóa Đơn -> Folder 2
      let invoiceCats: InvoiceCategory[] = [];
      try {
        invoiceCats = await invoiceApi.getCategories(selectedMonth, selectedYear);
      } catch (e) {
        console.warn('Lỗi tải danh mục hóa đơn:', e);
      }

      if (invoiceCats && invoiceCats.length > 0) {
        setZipInvoiceCategories(invoiceCats);
        setZipProgress({
          current: employees.length + 2,
          total: employees.length + 2,
          name: '📸 Bảng Tổng Hợp Chi Phí Hóa Đơn & Nhập Hàng',
        });
        await new Promise((r) => setTimeout(r, 150));

        if (offscreenInvoiceSummaryRef.current && invoiceFolder) {
          const invoiceBlob = await captureElementToBlob(offscreenInvoiceSummaryRef.current, 2.5);
          if (invoiceBlob) {
            invoiceFolder.file(
      ...
      setZipProgress({
        current: employees.length + 2,
        total: employees.length + 2,
        name: '📦 Đang nén 2 thư mục vào file ZIP...',
      });
      ...
      a.href = url;
      a.download = `Tron_Bo_Bao_Cao_Luong_Va_Hoa_Don_Tang2_Thang_${selectedMonth}_${selectedYear}.zip`;
      ...
      showToast(`📦 Đã tải file ZIP trọn bộ gồm 2 thư mục: [1_Luong_Nhan_Vien] và [2_Hoa_Don_Thang] gửi sếp thành công!`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {/* 3. Offscreen Invoice Summary Table for Folder 2 */}
        {zipInvoiceCategories && zipInvoiceCategories.length > 0 && (
          <div
            ref={offscreenInvoiceSummaryRef}
            className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-300 select-none text-slate-800 mt-6"
            style={{ width: '1050px' }}
          >
            {/* Header cho in ấn & chụp ảnh gửi sếp */}
            <div className="mb-4 pb-3 border-b-2 border-emerald-900/30 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-800 text-white font-black flex items-center justify-center text-sm shadow-sm">
                  T2
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    Kỳ hạch toán: Tháng {selectedMonth}/{selectedYear}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-xs font-bold text-slate-700">
                  ...
                  <span className="text-emerald-800 font-black">
                    {zipInvoiceCategories
                      .reduce((sum, c) => sum + (c.fixedAmount || c.totalAmount || 0), 0)
            ...
            <table className="w-full border-collapse border-2 border-gray-600 text-sm">
              <thead>
                <tr className="bg-[#245839] text-white font-extrabold select-none">
                  <th className="border border-gray-500 py-3 px-3 text-center w-16">순번</th>
                  <th className="border border-gray-500 py-3 px-4 text-center">품목 (Hạng mục)</th>
                  <th className="border border-gray-500 py-3 px-4 text-center min-w-[130px]">금액 (Số tiền)</th>
                  <th className="border border-gray-500 py-3 px-3 text-center w-20">결제</th>
                </tr>
              </thead>
              <tbody>
                {zipInvoiceCategories.map((cat, idx) => {
                  const total = cat.fixedAmount || cat.totalAmount || 0;
                  return (
                    <tr key={cat.id}>
                      ...
                      <td className="border border-gray-500 py-2.5 px-3 text-center bg-white">
                        <div className="flex items-center justify-center">
                          {cat.isPaid ? (
                            <div className="w-5 h-5 bg-emerald-700 text-white rounded-sm flex items-center justify-center text-xs font-black">
                              ✓
                            </div>
                          ) : (
                            <div className="w-5 h-5 border-2 border-gray-700 rounded-sm bg-white"></div>
                          )}
                        </div>
                      </td>
                    </tr>
                ...
                {/* Grand Total */}
                <tr className="bg-white font-black text-sm">
                  <td colSpan={2} className="border border-gray-500 py-3 px-4 text-center bg-[#ff0000] text-black font-black text-base">
                    Tổng
                  </td>
            ...
            {/* Signatures */}
            <div className="mt-8 pt-4 border-t border-slate-200 grid grid-cols-2 gap-4 text-center text-xs">
              <div>
                <div className="font-bold text-slate-800">Kế toán nhà hàng</div>
                <div className="text-[11px] text-slate-400 mt-0.5">(Ký & ghi rõ họ tên)</div>
                <div className="h-12"></div>
              </div>
              <div>
